...
const lib = {
  v2: THREE.Vector2,
  noise: {
    simplex3: Utils.math.v3.noise3d, // createNoise3D(Date.now()),
  },
...
class Particle {
  ...
  angleTo(p) {
    return Math.atan2(p.y - this.position.y, p.x - this.position.x)
  }
    ...
    for (let i = this.points.length - 1; i >= 0; i--) {
      ...
      if (point.reset) {
        ...
        theta
                = noiseVal
                * TAU
                * (1000 / (point.position.distanceTo(this.canvas.center) + 1000))
        theta2 = point.angleTo(this.canvas.center) + HALF_PI * 0.25

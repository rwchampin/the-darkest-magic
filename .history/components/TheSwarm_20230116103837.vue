...
const lib = {
  v2: THREE.Vector2,
  noise: {
    simplex3: createNoise3D(Date.now()),
  },
...
class ParticleSystem {
  ...
  update() {
    ...
    if (this.points.length < this.config.max && this.tick % 2 === 0) {
      ...
      let x, y
      if (rand) {
        x = Math.round(Math.random()) * this.canvas.dimensions.x
        y = Math.round() * this.canvas.dimensions.y
      }
      else {
        x = Math.random() * this.canvas.dimensions.x
        y = Math.round(Math.random()) * this.canvas.dimensions.y
      }

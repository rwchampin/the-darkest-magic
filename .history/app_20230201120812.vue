...
onMounted(() => {
  ...
  gsap.ticker.fps(3)
  gsap.ticker.add(() => {
    const delta = clock.getDelta() * 0.00001

    pointLight.position.x = Math.random() * Math.sin((Math.PI * 2) * delta) * 10
    pointLight.position.y = Math.random() * Math.cos((Math.PI * 2) * delta) * 10

...
let points, mousePointLight; let scene; let camera; let renderer; let ambientLight; let pointLight; let directionalLight; let rectAreaLight; let spotLight; let hemisphereLight

const { debugMode } = nuxtApp.$appStore

function mouseMe() {
  const raycastPlaneGeometry = new THREE.PlaneGeometry(100, 100)
  const raycastPlaneMaterial = new THREE.MeshBasicMaterial({ color: 'orange', side: THREE.DoubleSide })
  ...
  scene.add(raycastPlane, mousePointLight)
...
//     const { x, y, z } = Utils.animation.getCircleCoords({ time: performance.now(), angle: Utils.math.randomInteger(10, 20) })
//   })
// }
// const createPoints = () => {
//   points = []
...
//   //   _x: x, _y: y, _ctx: renderer.getContext(),
//   // })
//   // p.draw()
//   // }

//   // gsap.ticker.add(animatePoints)
// }
...
onMounted(() => {
  ...
  gsap.ticker.add((time, deltaTime, frame) => {
    pointLight.position.set(...Utils.animation.getCircleCoords(time, 10.5))

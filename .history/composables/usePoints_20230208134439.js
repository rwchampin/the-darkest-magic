...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    const updateParticles = () => {
        ...
        //     debugger;
            ...
            vp[x] = radius * Math.cos(theta) * Math.sin(phi)
            vp[x+1] = radius * Math.sin(theta) * Math.sin(phi)
            vp[x+2] = radius * Math.cos(phi);

            vp[x] =  vp[x] + Utils.noise.perlin3(vp[x], vp[x+1],vp[x+2]) * THREE.MathUtils.randFloatSpread(.5)
            vp[x+1] = y0 + Utils.noise.simplex3(vp[x], vp[x+1],vp[x+2]) * THREE.MathUtils.randFloatSpread(.5)
            vp[x+2] = z0 + Utils.noise.simplex3(vp[x], vp[x+1],vp[x+2])* THREE.MathUtils.randFloatSpread(.5)

...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const t = clock.getElapsedTime();
        ...
        const theta = THREE.MathUtils.randFloatSpread(360) * t
      const phi = THREE.MathUtils.randFloatSpread(360) * t
      const x = Math.random() * radius *10* Math.cos(theta) * Math.sin(phi * t)
      const y = Math.random() *radius * 5* Math.sin(theta) * Math.sin(phi * t)
      const z = Math.random() *radius * Math.cos(phi)
   ...
   geometry.setAttribute( 'position', new THREE.Float32BufferAttribute( positions, 3 ) );
    ...
    const updateParticles = () => {
        ...
        for (let x = 0; x < count; x+=3) {
            ...
            const theta =  THREE.MathUtils.clamp(t,0,360)
            const phi = THREE.MathUtils.clamp(t,0,360)
        //     debugger;

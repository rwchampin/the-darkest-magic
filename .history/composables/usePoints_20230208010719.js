...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    let material, vertices = [], sizes = [], colors = [], particles;
    ...
    const shaderMaterial = new THREE.ShaderMaterial({
        uniforms: {
            ...
            sampler2D: { value: new THREE.TextureLoader().load('~/assets/particles/particleMask.png')}
        },
        vertexShader: `
...
geometry.computeBoundingBox()
    material = new THREE.PointsMaterial({  size: .051, vertexColors: true, transparent: true });
    ...
    const update = () => {
        ...
        for (let x = 0; x < positions.length; x+3) { 
            ...
            const phi = Math.acos(2 * Math.random() - 1);
            debugger;
            positions[x] = radius * Math.cos(t + theta) * Math.sin(phi)
            positions[x + 1] = radius * Math.sin(t + theta) * Math.sin(phi)
            positions[x + 2] = radius * Math.cos(t + phi)
        }

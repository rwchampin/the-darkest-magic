...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    let sunGeometry, sunMaterial, sun;
    onMounted(() => {
    const texture = new THREE.TextureLoader().load(soulUrl)
    let controls,electrons = [], nucleus, numElectrons = 100; // more electrons = slower updating
    ...
    sunGeometry = new THREE.IcosahedronGeometry(10, 30);
        sunMaterial = new THREE.MeshMatcapMaterial({
            matcap: texture,
            fog: true,
        flatShading: true
    })

...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
  ...
  const updateParticles = () => {
    ...
    for (let i = 0; i < c; i++) {
      ...
      const theta = Math.atan2(y, x) + 0.01 * Math.random()
      const phi = Math.acos(z / r) + 0.01 * Math.random()

      vec.applyAxisAngle(new THREE.Vector3(Math.random() * Math.PI, Math.random(), Math.random()), Math.random() * Math.PI * 2)

      ps[i * 3] = vec.x
      ps[i * 3 + 1] = vec.y
      ps[i * 3 + 2] = vec.z

      // const newTheta = theta + 0.01 * Math.random()
      // const newPhi = phi + 0.01 * Math.random()

      // ps[i * 3] = 5 * Math.cos(newTheta) * Math.sin(newPhi)
      // ps[i * 3 + 1] = 5 * Math.sin(newTheta) * Math.sin(newPhi)
      // ps[i * 3 + 2] = 5 * Math.cos(newPhi)
    }

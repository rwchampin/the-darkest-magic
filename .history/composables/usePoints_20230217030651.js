...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
  ...
  const updateParticles = () => {
    ...
    for (let i = 0; i < vectors.length; i++) {
      ...
      ps[i * 3] = vector.x + Utils.noise.perlin3(vector.x, vector.y, vector.z) * 0.1
      ps[i * 3 + 1] = vector.y + Utils.noise.perlin3(vector.x, vector.y, vector.z) * 0.1
      ps[i * 3 + 2] = vector.z + Utils.noise.perlin3(vector.x, vector.y, vector.z) * 0.1
    }

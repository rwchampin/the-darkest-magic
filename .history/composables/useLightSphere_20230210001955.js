...
import { init } from '~~/particles/magic-mouse-particles/3d/fboHelper'
export const useLightSphere = ({scene, camera, renderer }) => {
  const start = Date.now();
  let container, _width, _height, mat;
  const init = () => {
    createWorld();
    createLights();
    //createGrid();
...
function createWorld() {
	...
	//---
	camera = new THREE.PerspectiveCamera(35, _width / _height, 1, 100);

...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
  ...
  material = new THREE.PointsMaterial({
    ...
    for (let i = 0; i < 10000; i++) {
        ...
        let vertex = new THREE.Vector3(px,py,pz);
...
let theta1 = 0, phi1 = 0; 
    const updateParticles = () => {
        ...
        for (let q = 0; q < ps.length; q+=3) {
const val = Math.random() * (Math.PI*2);

            const v1 = new THREE.Vector3(ps[q], ps[q+1], ps[q+2])
            const elapsedTime = clock.getElapsedTime() * .0001;
           



            // const t = THREE.MathUtils.clamp(elapsedTime, 0, 1);

            // const lx = THREE.MathUtils.lerp(v1.x, x0, t)
            // const ly = THREE.MathUtils.lerp(v1.y, y0, t)
            // const lz = THREE.MathUtils.lerp(v1.z, z0, t)

            // gsap.to(v1, {x: Math.random() * (Math.PI*2), y: Math.random() * (Math.PI*2), z: Math.random() * (Math.PI*2), duration: 300 })
            ps[q] += THREE.MathUtils.clamp(elapsedTime*Math.random(), 0, Math.PI*2) 
            ps[q+1] += THREE.MathUtils.clamp(elapsedTime*Math.random(), 0, Math.PI*2) 
            ps[q+2] += THREE.MathUtils.clamp(elapsedTime*Math.random(), 0, Math.PI*2) 
        }

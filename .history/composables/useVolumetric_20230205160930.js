...
const useVolumetric = ({ scene, camera, ctx, canvas2d }) => {
  ...
  const s = new THREE.BoxGeometry(1, 1, 1);
  const w = new THREE.MeshBasicMaterial({ color: 0x000000 });
  const d = new THREE.Mesh(s, w);
  mesh.position.set(0, 0, 0);
  camera.position.z = 100
  scene.add(d);

  var introCanvas = document.createElement('canvas');
  var context = introCanvas.getContext('2d');
  ...
  // document.body.appendChild(introCanvas);
    ...
    var x0, y0, z0;
      ...
      // timer = setInterval(updateVolumetricParticles, 10 / 24);


    function updateVolumetricParticles() {
  ...
  return updateVolumetricParticles
};

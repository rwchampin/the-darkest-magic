  ...
  loader.load(logoUrl, (gltf) => {
    ...
    logo.traverse((child) => {
      if (child.geometry) {
        ...
        const { ge } = fillGeometryWithPoints(g, 1000)
        const t = new THREE.Points(ge, new THREE.PointsMaterial({ color: 0xFFFFFF, size: 0.1 }))
      }
    })
    const points = Utils.three.createPoints({ count: 2000, size: Math.random() * 3, color: 'blue' })
    debugger

...
export const useVolumetric = ({ scene, camera, renderer }) => {
  ...
  onMounted(() => {
    ...
    const animatePointsAroundSphere = () => {
      const shere = new THREE.SphereGeometry(1, 32, 32)
      const points = new THREE.Points(shere, new THREE.MeshBasicMaterial({ color: 0xFFFFFF }))
      const count = points.geometry.attributes.position.count
      ...
      for (let i = 0; i < count; i++) {
        ...
        const i3 = i * 3
        const x = Math.sin(angle) * positions[i3]
        const y = Math.cos(angle) * positions[i3 + 1]
        const z = Math.cos(angle) * positions[i3 + 2]
        const scale = scales[i]
        const vec = Utils.noise.simplex3(x / 10, y / 10, z / 10)
        positions[i3] = x + vec * scale
        positions[i3 + 1] = y + vec * scale

...
export const useLightSphere = ({ scene, camera, renderer }) => {
  onMounted(() => {
    const cubeMap = createCubeMap()

    const light = new THREE.DirectionalLight(0xFFFFFF, 1.75)
    light.position.setScalar(1)
    scene.add(light, new THREE.AmbientLight(0xFFFFFF, 0.25)); z
    ...
    const g = new THREE.IcosahedronGeometry(10, 70)
    const localUniforms = {
...
float cnoise(vec4 P){
  ...
  return 2.2 * n_xyzw;
    ...
    const bCam = new THREE.Camera()
    const bScn = new THREE.Scene()
    const bQuad = new THREE.Mesh(

...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    let material, prevVectorPositions = [],vectorPositions = [], positions = [], sizes = [], colors = [], particles;
    ...
    for (let i = 0; i < 40; i++) {
      ...
      const z = radius * Math.cos(phi)
        ...
        vec.y = y;
        vec.z = z;
        vectorPositions.push(vec.clone())
    ...
    prevVectorPositions = [...vectorPositions]
    ...
    const count = vectorPositions.length;
    const updateParticles = () => {
        geometry.attributes.position.array = []
        for (let i = 0; i < count; i+=3) {
            const t = clock.getElapsedTime() * .001;
            const prev = new THREE.Vector3(ps[i], ps[i+1], ps[i+2])

            const theta = THREE.MathUtils.randFloatSpread(360);
            const phi =THREE.MathUtils.randFloatSpread(360);
        //     debugger;

            vp[i].x = radius * Math.cos(theta) * Math.sin(phi)
            vp[i].y = radius * Math.sin(theta) * Math.sin(phi)
            vp[i].z = radius * Math.cos(phi);

            vp[i].x =  vp[i].x + Utils.noise.perlin3(vp[i].x, vp[i].y,vp[i].z) * THREE.MathUtils.randFloatSpread(.5)
            vp[i].y = vp[i].y + Utils.noise.simplex3(vp[i].x, vp[i].y,vp[i].z) * THREE.MathUtils.randFloatSpread(.5)
            vp[i].z = vp[i].z + Utils.noise.simplex3(vp[i].x, vp[i].y,vp[i].z)* THREE.MathUtils.randFloatSpread(.5)

            const ll = cp.lerp(vp[i], .5)

            vp[i].x = ll.x
            vp[i].y = ll.y
            vp[i].z = ll.z
            geometry.attributes.position.array.push(vp.x, vp.y, vp.z)
        }

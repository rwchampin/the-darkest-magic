   ...
   geometry.setAttribute( 'position', new THREE.Float32BufferAttribute( positions, 3 ) );
    ...
    const updateParticles = () => {
        ...
        for (let x = 0; x < count; x+=3) {
            ...
            ps[x] = .001*(radius) * Math.PI*Math.sin(180/Math.PI * 2 * Math.random())  
            ps[x + 1] = .001*(radius) * Math.PI*Math.cos(180/Math.PI * 2 * Math.random())
            ps[x + 2] = (radius) * Math.cos(180/Math.PI * 2 * Math.random())

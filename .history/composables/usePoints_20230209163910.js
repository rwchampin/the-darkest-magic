...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    let positions = [], sizes = [], colors = [], particles;
...
//              const y0 = radius * Math.sin(theta) * Math.cos(phi);
//              const z0 = radius * Math.cos(theta)
        ...
        // }
        var distance = 100;
var egeometry = THREE.Geometry();

for (var i = 0; i < 1000; i++) {

  var vertex = new THREE.Vector3();

  var etheta = THREE.MathUtils.randFloatSpread(360);
  var ephi = THREE.MathUtils.randFloatSpread(360);

  vertex.x = distance * Math.sin(etheta) * Math.cos(ephi);
  vertex.y = distance * Math.sin(etheta) * Math.sin(ephi);
  vertex.z = distance * Math.cos(etheta);

  egeometry.vertices.push(vertex);
}
var eparticles = new THREE.Points(egeometry, new THREE.PointMaterial({
  color: 0xffffff,size: 2
}));
eparticles.boundingSphere = 50;

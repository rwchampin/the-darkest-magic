...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    const createNucleus = () => { 
        ...
        nucleus = new THREE.Mesh(nucGeo, nucMaterial)
        nucleus.position.set(0, 0, 0);
        scene.add(nucleus)
       
    }
    const pos = nucleus.geometry.attributes.position.array;
    const nucLength = nucleus.geometry.attributes.position.array.length
    const updateNucleus = () => {
        
        for (let l = 0; l < pos.length; l + 3) {
            let time = Date.now();

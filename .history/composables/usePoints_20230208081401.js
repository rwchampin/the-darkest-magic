...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
   ...
   geometry.setAttribute( 'position', new THREE.Float32BufferAttribute( positions, 3 ) );
    ...
    const updateParticles = () => {
        ...
        //     const theta = 2 * Math.sin();
        //     const phi = Math.acos(2 * Math.random() - 1);
        // //     debugger;

        //     var s = Math.random() < 0.5 ? -1 : 1;
        //     ps[x] = (radius) * Math.cos(theta *t) * Math.sin(phi)
        //     ps[x + 1] = (radius) * Math.sin(theta*t) * Math.sin(phi)
        //     ps[x + 2] = (radius) * Math.cos(phi)

         // found at http://stackoverflow.com/questions/12732590/how-map-2d-grid-points-x-y-onto-sphere-as-3d-points-x-y-z
        // longitude = x / R
        // latitude = 2 * atan(exp(y/R)) - pi/2

        var latitude = ps[x] / radius;
        var longitude = (2 * Math.atan(Math.exp(ps[x+1] / radius))) - (Math.PI / 2);

        xCoord = radius * Math.cos(latitude) * Math.cos(longitude);
        yCoord = radius * Math.cos(latitude) * Math.sin(longitude);
        zCoord = radius * Math.sin(latitude);

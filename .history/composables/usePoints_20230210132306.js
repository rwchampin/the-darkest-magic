...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    let material,,particlesDelay = 50, positions = [], sizes = [], colors = [], particles;
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const y = radius * Math.sin(theta) * Math.sin(phi)
        const z = radius * Math.cos(phi)
         ...
         let vertex = new THREE.Vector3(radius/2, 0, 0);
...
let theta1 = 0, phi1 = 0
    const updateParticles = () => {
        ...
        for (let x = 0; x < count; x+=3) {
            const t = .0001;
            const vec = new THREE.Vector3(ps[x], ps[x+1], ps[x+2])
             vec.applyAxisAngle(vec.rotationAxis, 0.01); 
        }

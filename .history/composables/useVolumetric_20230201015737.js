...
export const useVolumetric = ({ scene, camera, renderer }) => {
  onMounted(() => {
    ...
    function setupScene() {
      ...
      for (let i = 0; i < 500; i++) {
        const rng = { x: Math.random(), y: Math.random() }
        const lat = Math.acos(2 * rng.x - 1) - Math.PI / 2
        const lng = 2 * Math.PI * rng.y

        vertices[i] = [
          SCL * Math.cos(lat) * Math.cos(lng),
          SCL * Math.cos(lat) * Math.sin(lng),
          SCL * Math.sin(lat),
        ]
      }
      ...
      const mesh = new THREE.Mesh(bufferGeometry, vmaterial)
      debugger
    }
    ...
    function setupPostprocessing() {
      ...
      // const volumetericLightShaderUniforms = pass.uniforms

...
import m from '~/assets/particles/dotTexture.png'
// import { useVolumetric } from './useLightSphere'
import { useMouse } from '@vueuse/core'
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    const mx = useMouse().x;
    const my = useMouse().y
const dgeometry = new THREE.BoxGeometry( 1, 1, 1 );
const dmaterial = new THREE.MeshBasicMaterial( {color: 0x00ff00} );
const cube = new THREE.Mesh( dgeometry, dmaterial );
scene.add( cube );
    let material, prev = [], positions = [], sizes = [], colors = [], particles;
    let windowHalfX = window.innerWidth / 2;
    let windowHalfY = window.innerHeight / 2;
    const clock = new THREE.Clock();
    scene.fog = new THREE.FogExp2(0x000000, 0.001);
    const colorList = chroma.scale(["black", '#222', '#111', "black"]).colors(20);
    const geometry = new THREE.BufferGeometry();
    const tex = new THREE.TextureLoader().load(m)
    // const updateVolumetric = useVolumetric({scene, camera, renderer})
    ...
    for (let i = 0; i < 1000; i++) {
      ...
      const theta =  Math.sin(2 * Math.PI);
      const phi = Math.acos(2 * -t - 1);
      const x = 1.*(radius * Math.cos(theta) * Math.sin(phi))
      const y = .1*(radius * Math.sin(theta) * Math.sin(phi))
      const z = .1*(radius * Math.cos(phi))
   ...
   geometry.setAttribute( 'position', new THREE.Float32BufferAttribute( positions, 3 ) );
    ...
    let theta = 0;
            let r = 100;
    let n = 1
            const gneometry = new THREE.BoxGeometry( 1, 1, 1 );
const mnaterial = new THREE.MeshBasicMaterial( {color: 0x00ff00} );
const ccube = new THREE.Mesh( gneometry, mnaterial );
scene.add( ccube );
    const updateParticles = () => {
        ...
        for (let x = 0; x < count; x+=3) {
            ...
            // const v1 = new THREE.Vector3(ps[x], ps[x+1], ps[x+2]).normalize()
            // const theta = THREE.MathUtils.randFloatSpread(360);
            // const phi = THREE.MathUtils.randFloatSpread(360);
        //     debugger;

            // const x0 = Math.sin(t*Math.PI) + radius + Math.cos(theta) * Math.sin(phi)
            // const y0 =Math.sin(t) + radius + Math.sin(theta) * Math.sin(phi)
            // const z0 = Math.sin(t) + radius + Math.cos(phi);

            // let finalX = x0 + Utils.noise.simplex3(x0, y0,z0) * THREE.MathUtils.randFloatSpread(.5+t)
            // let finalY = y0 + Utils.noise.simplex3(x0,y0,z0) * THREE.MathUtils.randFloatSpread(.5+t)
            // let finalZ = z0 + Utils.noise.simplex3(x0,y0,z0)* THREE.MathUtils.randFloatSpread(.5+t)

            // const v2 = new THREE.Vector3(finalX, finalY, finalZ).normalize()

            // const lx = THREE.MathUtils.lerp(v1.x, v2.x, THREE.MathUtils.clamp(t,0,1))
            // const ly = THREE.MathUtils.lerp(v1.y, v2.y, THREE.MathUtils.clamp(t,0,1))
            // const lz = THREE.MathUtils.lerp(v1.z, v2.z, THREE.MathUtils.clamp(t,0,1))

            // const newVec = new THREE.Vector3(lx,ly,lz) 
           ...
           const cc = ps[x] + r * Math.cos(theta)
           const rr = ps[x + 1] + r * Math.sin(theta)
            const hh = ps[x + 2] + r * Math.cos(theta)
            debugger;
        }

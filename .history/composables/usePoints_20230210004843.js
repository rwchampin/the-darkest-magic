...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    for (let i = 0; i < 4000; i++) {
      ...
      theta = 2 * Math.PI * deltaTime * 0.00001;
      phi = Math.acos(2 * speed - 1) * elapsedTime * 0.00001;
   ...
   geometry.setAttribute( 'position', new THREE.Float32BufferAttribute( positions, 3 ) );
    ...
    const updateParticles = () => {
        ...
        for (let x = 0; x < count; x+=3) {
            ...
            theta0 = THREE.MathUtils.clamp((Math.PI * 2) * elapsedTime, 0, 360);
            phi0 = THREE.MathUtils.clamp((Math.PI * 2) * elapsedTime, 0, 360);

            const x0 = radius * Math.sin(theta0) * Math.cos(phi0) * Math.random()
            const y0 = radius * Math.sin(theta0) * Math.sin(phi0) * Math.random()
            const z0 = radius * Math.cos(phi0) * Math.random()

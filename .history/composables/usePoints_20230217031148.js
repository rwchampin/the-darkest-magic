...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
  ...
  const addNoise = vector => Utils.noise.simplex3(vector.x * Math.random(), vector.y * Math.random(), vector.z * Math.random())
  ...
  const updateParticles = () => {
    ...
    for (let i = 0; i < vectors.length; i++) {
      ...
      ps[i * 3] = vector.x + addNoise(vector)
      ps[i * 3 + 1] = vector.y + addNoise(vector)
      ps[i * 3 + 2] = vector.z + addNoise(vector)
    }

...
const useLightsFN = () => {
  ...
  const createPointLight = (color: ColorRepresentation | undefined, intensity: number | undefined: number | undefined: number, distance: number, decay: number) => {
    pointLight = new THREE.PointLight(color, intensity, distance, decay)
    return pointLight
  }

  const createRectAreaLight = (color: ColorRepresentation | undefined, intensity: number | undefined: number | undefined: number, width: number, height: number) => {

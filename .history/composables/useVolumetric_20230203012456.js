...
export const useVolumetr  ic = ({ scene, camera, renderer }) => {
  ...
  onMounted(() => {
    ...
    function setupScene() {
      ...
      const sgeometry = new THREE.SphereGeometry(1, 16, 16)
      ...
      const bgeometry = new THREE.BoxGeometry(1, 1, 1, 2, 2, 2)
      ...
      const particles = []
      for(let i = 0; i < 100; i++) {
        prticles.push(new ChaosParticle(window.innerWidth * Math.random(), window.innerHeight * Math.random(), 0))
    }
    ...
    function setupPostprocessing() {
      ...
      volumetericLightShader = pass.uniforms

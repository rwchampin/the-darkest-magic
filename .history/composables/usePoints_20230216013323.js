...
let theta1 = 0, phi1 = 0
    const updateParticles = () => {
        ...
        for (let q = 0; q < ps.length; q+=3) {
            ...
            theta1 = Math.random() * 2 * Math.PI;
			phi1 = Math.acos(Math.random() * 2 - 1);

            const x0 = v1.x + radius * Math.cos(theta1) * Math.sin(phi1)
            const y0 = v1.y +  radius * Math.sin(theta1) * Math.sin(phi1)
            const z0 = v1.z + radius * Math.cos(phi1)

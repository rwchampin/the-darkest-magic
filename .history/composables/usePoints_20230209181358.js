...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    for (let i = 0; i < 4000; i++) {
      ...
      theta += 2 * Math.PI / speed;
      phi += Math.acos(2 * speed - 1);
   ...
   geometry.setAttribute( 'position', new THREE.Float32BufferAttribute( positions, 3 ) );
    ...
    const updateParticles = () => {
        ...
        for (let x = 0; x < count; x+=3) {
            ...
            theta0 = (Math.PI * 2)* Date.now() / 1000;
            phi0 = (Math.PI * 2)*Date.now() / 1000
            const x0 = radius * Math.sin(theta0*Math.random()) * Math.cos(phi0)
            const y0 = radius * Math.sin(theta0*Math.random()) * Math.sin(phi0)

...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
  ...
  material = new THREE.PointsMaterial({
    ...
    const radius = 5;
    const tits = [];
    theta.value = (2 * Math.PI) * Math.random(); phi.value = Math.acos(Math.random() * 2 - 1);
    for (let i = 0; i < 10000; i++) {

       theta.value = theta.value + .00000001
        phi.value = phi.value + .0000001;
        const px = radius * Math.cos(theta.value) * Math.sin(phi.value)
        const py = radius * Math.sin(theta.value) * Math.sin(phi.value)
        const pz = radius * Math.cos(phi.value)
 ...
 theta.value = theta.value + .000001
        phi.value = phi.value + .000001;

             ps[q] += radius * Math.cos(theta.value) * Math.sin(phi.value)
            ps[q + 1] += radius * Math.sin(theta.value) * Math.sin(phi.value)
            ps[q + 2] += radius * Math.cos(phi.value)
        }

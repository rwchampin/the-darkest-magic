...
import { Vector3 } from '~~/utils/Vectorpedia/Vector-saurus-rex'
import { Utils } from '~/utils/'
import dotTexture from '~/assets/particles/dotTexture.png?url'

export const usePoints = ({ scene }) => {
  const particlesDelay = 50
  const MAX_PARTICLES = 5000
  const positions = new Float32Array(MAX_PARTICLES * 3)
  const sizes = new Float32Array(MAX_PARTICLES)
  const colors = new Float32Array(MAX_PARTICLES * 3)
  const vectors = new Float32Array(MAX_PARTICLES)
  ...
  for (let i = 0; i < MAX_PARTICLES; i++) {
    ...
    const vertex = new Vector3(px, py, pz)
    vertex.delay = Date.now() + (particlesDelay * i) - Math.random()
    vertex.rotationAxis = new Vector3(0, Math.random() * 2 - 1, Math.random() * 2 - 1)

...
import sUrl from '~/assets/particles/black-smoke-particle-3.png?url';
export const usePoints = ({ scene, camera, renderer }) => {

    const s = new THREE.TextureLoader().load(sUrl)
    let material, vertices = [], sizes = [], colors = [];
    let windowHalfX = window.innerWidth / 2;
    let windowHalfY = window.innerHeight / 2;
    const clock = new THREE.Clock();
    scene.fog = new THREE.FogExp2(0x000000, 0.001);
    const colorList = chroma.scale(['gray', 'black']).colors(20);
    ...
    for (let i = 0; i < 10000; i++) {
      ...
      const x = radius * Math.cos(t + theta) * Math.sin(phi)
      const y = radius * Math.sin(t + theta) * Math.sin(phi)
      const z = radius * Math.cos(t + phi)

      vertices.push(x, y, z);
        sizes.push(Math.random() * 30)
        colors.push(colorList[Math.round(Math.random() * colorList.length)])
    }
    ...
    material = new THREE.PointsMaterial({ color: 0x000000, sizeAttenuation: true, vertexColors: true, alphaTest: 0.5, transparent: true });

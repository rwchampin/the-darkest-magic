...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    const texture = new THREE.TextureLoader().load(dotTexture)
...
let theta1 = 0, phi1 = 0
    const updateParticles = () => {
        ...
        for (let x = 0; x < tits.length; x++) {
            const vec = tits[x]

            theta1 = Math.random() * 2 * Math.PI;
			phi1 = Math.acos(Math.random() * 2 - 1);
            // vec.applyAxisAngle(vec.rotationAxis, THREE.MathUtils.clamp(THREE.MathUtils.degToRad(x), 0, Math.PI*2)); 
            // vec.randomDirection().multiplyScalar(Math.PI*2)
            const y = vec.setFromSphericalCoords(radius, phi1,theta1)

...
import s from '~/assets/particles/dotTexture.png?url';
export const usePoints = ({ scene, camera, renderer }) => {
    const vertices = [];
    let scalarVertices = [], vertColors = [], points;
    ...
    gravity = Utils.math.randomFloat(-1,1)//-0; //try changing to a positive number (not too large, for example 0.3), or negative for floating upwards.
    ...
    function draw() {
        ...
        for (let i = 0; i < 2000; i++) {
            ...
            x0 = sphereRad * Math.sin(phi) * Math.cos(theta* t);
            y0 = sphereRad * Math.sin(phi) * Math.sin(theta * t);
            ...
            vertices.push(p.accelX, p.accelY, p.accelZ);
            // const vec = new THREE.Vector3(p.x, p.y, p.z);
            // vec.multiplyScalar(1.5);
            // scalarVertices.push(vec.x, vec.y, vec.z);
        }

        pointsGeometry.setAttribute('position', new THREE.Float32BufferAttribute(vertices, 3));
        // pointsGeometry.setAttribute('color', new THREE.Float32BufferAttribute(vertices, 3));
       // pointsGeometry.setAttribute('scalar', new THREE.Float32BufferAttribute(scalarVertices, 3));
        // pointsGeometry.setAttribute('color', new THREE.Float32BufferAttribute(vertColors, 1));
        points = new THREE.Points(pointsGeometry, pointsMaterial);
        const b = Utils.debug.createBox();
        b.position.x = 5
        const d = Utils.debug.createBox();
        d.position.set(1, 1, 0)
        d.material.color = 0xFF0000
        scene.add(points, b,d);

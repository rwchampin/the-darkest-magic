...
import blkUrl from '~/assets/matcaps/big-blog-matcap.png?url';
...
export const usePoints = ({ scene, camera, renderer, nuxtApp }) => {
    ...
    const tex = loader.load(m)
    const blk = loader.load(blkUrl)
    let nuc, nucPos,nucLength;
    const createNucleus = () => { 
        debugger
        const nucGeo = new THREE.IcosahedronGeometry(1, 30)
        const nucMaterial = new THREE.MeshPhongMaterial({
            map: blk,
            color: 0xff0000,
            emissive: 0x000000,
            specular: 0x111111,
        ...
        nuc = new THREE.Mesh(nucGeo, nucMaterial)
        nuc.position.set(0, 0, 0);
        scene.add(nuc)
        nucPos = nuc.geometry.attributes.position.array;
    nucLength = nuc.geometry.attributes.position.array.length
    }

    
    const updateNucleus = () => {
        
        for (let l = 0; l < nucLength; l += 3) {
            const theta = 2 * Math.PI * Math.random();
      const phi = Math.acos(2 * Math.random() - 1);
            const x =  Utils.noise.perlin3(nucPos[l], nucPos[l + 1], nucPos[l + 2])
            const y = Utils.noise.perlin3(nucPos[l], nucPos[l + 1], nucPos[l + 2])
            const z =  Utils.noise.perlin3(nucPos[l], nucPos[l + 1], nucPos[l + 2])

            nucPos[l] += Math.sin(x* .1)
            nucPos[l + 1] += Math.cos(y*.1)
            nucPos[l + 2] += z*.1
        }
        nuc.geometry.computeVertexNormals();
        nuc.geometry.computeFaceNormals();
        nuc.geometry.attributes.position.needsUpdate = true;
    }
    createNucleus()
    ...
    const updateParticles = () => {
        ...
        //     debugger;
            ...
            let finalX = x0 + Utils.noise.perlin3(x0, y0,z0) * THREE.MathUtils.randFloatSpread(.5)
            let finalY = y0 + Utils.noise.simplex3(x0,y0,z0) * THREE.MathUtils.randFloatSpread(.5)
            let finalZ = z0 + Utils.noise.simplex3(x0,y0,z0)* THREE.MathUtils.randFloatSpread(.5)

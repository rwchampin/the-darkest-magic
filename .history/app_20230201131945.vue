...
const animatePoints = (points) => {
  points.forEach((point) => {
    const { x, y, z } = Utils.animation.getCircleCoords({ time, Utils.math.random(10,20) })
    point.rotation.x
...
onMounted(() => {
  ...
  gsap.ticker.add((time, deltaTime, frame) => {
    console.log('time', Math.sin(time * Math.PI))
    pointLight.position.set(...Utils.animation.getCircleCoords(time, 10.5))
    // pointLight.position.x = Math.random() / (Math.sin((Math.PI * 2) * (deltaTime * 0.001)) + 10)
    // pointLight.position.y = Math.random() / (Math.cos((Math.PI * 2) * (deltaTime * 0.001)) + 10)

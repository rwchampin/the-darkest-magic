...
export default function createIcosahedron() {
  ...
  const icosahedron = new THREE.Mesh(icosGeometry, BlackGooBallMaterial)
  const animateIcosahedron = () => {
    // icosahedron.rotation.x += 0.1
    // icosahedron.rotation.y += 0.1
    icosahedron.material.uniforms.u_time.value = clock.getElapsedTime()
    icosahedron.material.uniforms.u_resolution.value = new THREE.Vector2(window.innerWidth, window.innerHeight)

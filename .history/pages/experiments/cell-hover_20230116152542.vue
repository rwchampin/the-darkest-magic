...
function asteroidGen() {
   const   x = randBetween(55, 250),
         y = randBetween(55, 250),
         rad = randBetween(3, 20),
         dur = randBetween(7000, 9000),
         op = randBetween(0,10) / 10,
         theme = randBetween(0, colors.cell.length - 1),
         stroke = colors.cell[theme].stroke[randBetween(0, colors.cell[theme].stroke.length - 1)],
         fill =  colors.cell[theme].fill[randBetween(0, colors.cell[theme].fill.length - 1)],
         path = draw.path(ellipseToPath(coo.width / 2, coo.height / 2, x, y))
            .attr({
               stroke: 'transparent',
               fill: 'transparent'
            }),
         ln = path.length(),
         asteroid = draw.circle(rad).attr({ 
            'fill': fill,
            'stroke': stroke,
            'stroke-width': 2,
            'cx': coo.width / 2, 
   ...
   grp.backward()
   if(Math.random() < 0.5) grp.flip('x', {x: coo.width / 2, y: coo.height / 2})
}
 </script>

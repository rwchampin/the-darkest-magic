...
class ParticleSystem {
  constructor(opts) {
    ...
    this.canvas = this.experiece.ctx
    this.canvas.resize()
  ...
  update() {
    ...
    for (let i = this.points.length - 1; i >= 0; i--) {
      ...
      if (p.reset) {
        ...
        size = (p.position.y / this.canvas.dimensions.y * this.config.size) ** 2 + 2
...
onMounted(() => {
  debugger
  const experiece = new Experience()
...
/* .canvas {background-image: radial-gradient(#03090f, #010206); position: absolute; top: 0 ;left: 0 ;width: 100vw ;height: 100vh;} */
</style>

...
// import { useMagicMouse } from '~~/core/useMagicMouse.js'
...
const getMousePosition = (event) => {
  const rect = canvas2d.value.getBoundingClientRect()
  const x = event.clientX - rect.left
  const y = event.clientY - rect.top
  return { x, y }
}
const drawCursor = (r, color, ctx) => {
  debugger
  ctx.beginPath()
  ctx.arc(...getMousePosition(), r, 0, 2 * Math.PI)
  ctx.fillStyle = color
  ctx.fill()
}
function magicMouse(canvas) {
  const ctx = canvas.getContext('2d')
  debugger
  drawCursor(10, 'red', ctx)
}
const animate = () => {
  // animateIcosahedron()
...
onMounted(() => {
  ...
  const d = document.querySelector('.main-canvas-3d')
  ...
  camera = new CinematicCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000)
  ...
  // const makeIcos = createIcosahedron()
  // icosahedron = makeIcos.icosahedron
  // animateIcosahedron = makeIcos.animateIcosahedron
...
<template>
  <TheSplash v-if="!getAppLoadingStatus && !getAssetLoadingStatus" />
  <div v-else>
      ...
      <NMessageProvider>
        <n-notification-provider>
          <n-dialog-provider>
            ...
            <!-- <LazyTheDebugger v-if="getDebugMode" :renderer="renderer" /> -->

...
onMounted(() => {
  nextTick(() => {
    camera = useCore(nuxtApp)
    scene = useCore(nuxtApp)
    renderer = useCore(nuxtApp)
    ambientLight = useCore(nuxtApp)
    pointLight = useCore(nuxtApp)
    spotLight = useCore(nuxtApp)
    hemisphereLight = useCore(nuxtApp)
    ...
    scene.add(planet, stars, mesh, ambientLight, camera, pointLight, spotLight)
  })
})

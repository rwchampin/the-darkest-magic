...
// The methods add(), subtract(), multiply(), and divide() can all take either a Vector3 or a number as an argument.
  ...
  Vector2.prototype.getMagnitude() {
    /*******************
     * @method getMagnitude
     * @return {Number}
  ...
  Vector2.prototype.setMagnitude(t) {
    const e = this.getMagnitude()
    return e && t / e
  
  Vector2.prototype.fromPoints(t, e) {
    /*******************
     * @method fromPoints
     * @param {Vector2} t
     * @param {Vector2} e
  ...
  Vector2.prototype.fromObject(t) {
    /*****************
     * @method fromObject
     * @param {Object} t
     * @return {Vector2}
  ...
  Vector2.prototype.intercept(t) {
    console.log(-t * this.x + this.y)
    return -t * this.x + this.y
  }
  Vector2.prototype.midpoint(t) {
    const e = new Vector2(this.x + i.x, this.y + i.y)
    return e.divideScalar(2)
  }
  Vector2.prototype.randomize(min, max) {
    this.x = Math.random() * (max - min) + min
    this.y = Math.random() * (max - min) + min
    return this
  }
  Vector2.prototype.randomizeX(min, max) {
    this.x = Math.random() * (max - min) + min
    return this
  }
  Vector2.prototype.randomizeY(min, max) {
    this.y = Math.random() * (max - min) + min
    return this
  }
  Vector2.prototype.addRandom(min, max) {
    this.x += Math.random() * (max - min) + min
    this.y += Math.random() * (max - min) + min
    return this
  }
  Vector2.prototype.addRandomX(min, max) {
    this.x += Math.random() * (max - min) + min
    return this
  }
  Vector2.prototype.addRandomY(min, max) {
    this.y += Math.random() * (max - min) + min
    return this
  }
  Vector2.prototype.negative() {
    return new Vector2(-this.x, -this.y)
  }
  Vector2.prototype.add(v) {
    if (v instanceof Vector2)
      return new Vector2(this.x + v.x, this.y + v.y)
    else
      return new Vector2(this.x + v, this.y + v)
  }
  Vector2.prototype.addX(v) {
    return new Vector2(this.x + v, this.y)
  }
  Vector2.prototype.addY(v) {
    return new Vector2(this.x, this.y + v)
  }
  Vector2.prototype.addScalar(v) {
    return new Vector2(this.x + v, this.y + v)
  }
  Vector2.prototype.addScalarX(v) {
    return new Vector2(this.x + v, this.y)
  }
  Vector2.prototype.addScalarY(v) {
    return new Vector2(this.x, this.y + v)
  }
  Vector2.prototype.subtract(v) {
    if (v instanceof Vector2)
      return new Vector2(this.x - v.x, this.y - v.y)
    else
      return new Vector2(this.x - v, this.y - v)
  }
  Vector2.prototype.subtractX(v) {
    return new Vector2(this.x - v, this.y)
  }
  Vector2.prototype.subtractY(v) {
    return new Vector2(this.x, this.y - v)
  }
  Vector2.prototype.subtractScalar(v) {
    return new Vector2(this.x - v, this.y - v)
  }
  Vector2.prototype.subtractScalarX(v) {
    return new Vector2(this.x - v, this.y)
  }
  Vector2.prototype.subtractScalarY(v) {
    return new Vector2(this.x, this.y - v)
  }
  Vector2.prototype.multiply(v) {
    if (v instanceof Vector2)
      return new Vector2(this.x * v.x, this.y * v.y)
    else
      return new Vector2(this.x * v, this.y * v)
  }
  Vector2.prototype.multiplyX(v) {
    return new Vector2(this.x * v, this.y)
  }
  Vector2.prototype.multiplyY(v) {
    return new Vector2(this.x, this.y * v)
  }
  Vector2.prototype.multiplyScalar(v) {
    return new Vector2(this.x * v, this.y * v)
  }
  Vector2.prototype.multiplyScalarX(v) {
    return new Vector2(this.x * v, this.y)
  }
  Vector2.prototype.multiplyScalarY(v) {
    return new Vector2(this.x, this.y * v)
  },
  Vector2.prototype.divide(v) {
    if (v instanceof Vector2)
      return new Vector2(this.x / v.x, this.y / v.y)
    else
      return new Vector2(this.x / v, this.y / v)
  }
  Vector2.prototype.divideX(v) {
    return new Vector2(this.x / v, this.y)
  }
  Vector2.prototype.divideY(v) {
    return new Vector2(this.x, this.y / v)
  }
  Vector2.prototype.divideScalar(t) {
    // eslint-disable-next-line no-void, no-console
    return t === 0 ? void console.log('! Cannot divide by zero !') : (this.x /= t || 1, this.y /= t || 1, this)
  }
  Vector2.prototype.divideScalarX(v) {
    return new Vector2(this.x / v, this.y)
  }
  Vector2.prototype.divideScalarY(v) {
    return new Vector2(this.x, this.y / v)
  },
  Vector2.prototype.equals(v) {
    return this.x === v.x && this.y === v.y
  }
  Vector2.prototype.dot(v) {
    return this.x * v.x + this.y * v.y
  }
  Vector2.prototype.cross(v) {
    return new Vector2(this.x * v.y - this.y * v.x)
  }
  Vector2.prototype.length() {
    return Math.sqrt(this.dot(this))
  }
  Vector2.prototype.unit() {
    return this.divide(this.length())
  }
  Vector2.prototype.min() {
    return Math.min(Math.min(this.x, this.y))
  }
  Vector2.prototype.max() {
    return Math.max(Math.max(this.x, this.y))
  }
  Vector2.prototype.toAngles() {
    return -Math.atan2(-this.y, this.x)
  }
  Vector2.prototype.angleTo(t, i) {
  ...
  Vector2.prototype.angleToAlternative(a) {
    return Math.acos(this.dot(a) / (this.length() * a.length()))
  }
  Vector2.prototype.toArray(n) {
    return [this.x, this.y].slice(0, n || 2)
  }
  Vector2.prototype.clone() {
    return new Vector2(this.x, this.y)
  }
  Vector2.prototype.init(x, y) {
    this.x = x
    this.y = y
    return this
  }
...
// static method
Vector2.prototype.Vector2.random2D = function () {
  const length = Math.random()
  const angle = Math.random() * Math.PI * 2
  return new Vector2(Math.cos(angle) * length, Math.sin(angle) * length)

Vector2.prototype.Vector2.negative = function (a, b) {
  b.x = -a.x
  b.y = -a.y
  return b

Vector2.prototype.Vctor2.add = function (v1, v2) {
  Vector2.prototype.if (v1 instanceof Vector2 && v2 instanceof Vector2) {

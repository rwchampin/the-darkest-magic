  ...
  three: {
    createPoints: ({ geometry, count }) => {
      ...
      for (let i = 0; i < count * 3; i++) {
        const xx = geometry.attributes.position.array[i]
        const yy = geometry.attributes.position.array[i + 1]
        const zz = geometry.attributes.position.array[i + 2]

        const x = Utils.noise.simplex3(xx, yy, zz)
        const y = Utils.noise.simplex3(xx, yy, zz)
        const z = Utils.noise.simplex3(xx, yy, zz)
        vertices.push(x, y, z)
        debugger
      }
      debugger
      ...
      const material = new THREE.PointsMaterial({ size: 0.4, color: 0x888888 })

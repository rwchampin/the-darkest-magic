  ...
  color: {
    ...
    renderColorSwatchGrid: (colors=Utils.color.colorList) => {
      ...
      for (let i = 0; i < colors.length; i++) {
        const formattedColor = new THREE.Color(Utils.color.convertTo6DigitHex(colors[i]));
        if(formattedColor && typeof formattedColor === THREE.Color)
         swatchString += Utils.color.renderColorSwatch();
      }
    ...
    convertTo6DigitHex: (hexObj) => {
      let key = Object.keys(hexObj)[0];
      const hex = hexObj[key];

      if (hex.length === 4) {
        return hex.replace(/#([0-9a-f])([0-9a-f])([0-9a-f])/i, '#$1$1$2$2$3$3');
      }
      if (hex.length === 8) {
        return hex.replace(/#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})/i, '#$1$2$3');
      }
      if (hex.length === 7) {
        return hex.replace(/#([0-9a-f])([0-9a-f])([0-9a-f])/i, '#$1$1$2$2$3$3');
      }
      ...
      if (hex.length === 5) {
        return hex.replace(/#([0-9a-f])([0-9a-f])([0-9a-f])([0-9a-f])/i, '#$1$1$2$2$3$3$4$4');
      }
      if (hex.length === 3) {
        return hex.replace(/#([0-9a-f])([0-9a-f])([0-9a-f])/i, '#$1$1$2$2$3$3');
      }
      if (hex.length === 2) {
        return hex.replace(/#([0-9a-f])([0-9a-f])/i, '#$1$1$2$2$3$3');
      }
      if (hex.length === 1) {
        return hex.replace(/#([0-9a-f])/i, '#$1$1$2$2$3$3');
      }
      if (hex.length === 0) {
        return '#000000';
      }
      throw createError({
        statusMessage: `Invalid hex color passed to convertTo6DigitHex in Utils.color.convertTo6DigitHex()\nHex Value: ${key}:${hexObj[key]}\n Hex Length: ${hexObj[key].length}`,
        statusCode: 500,
      })
    },
    getColorsHSL: (color) => {

export const Noise = {
  createPerlinNoise2D(width, height, seed) {
    /******
     * Perlin noise in 2D, with the gradients at random points and the vectors from the
     * gradients to the sample point.
     * @param {number} width
     * @param {number} height
     * @param {number} seed: random seed for the gradients (the same seed will always generate the same noise)
     * @return {Array<number>} noise: the noise values, in a 1D array
     * @example const noise = Noise.createPerlinNoise2D(256, 256, 0)

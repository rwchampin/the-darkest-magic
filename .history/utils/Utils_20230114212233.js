...
export const Utils = {
  ...
  math: {
    ...
    v3: {
      ...
      createPerlinNoiseFields: (v1, v2, v3, v4) => {
        const x = Utils.maths.v3.get(
          Utils.math.noise2d(v1.x, v1.y),
          Utils.math.noise2d(v2.x, v2.y),
          Utils.math.noise2d(v3.x, v3.y),
        )
      ...
      createPerlinNoiseField: (v1, v2, v3) => {
        const x = Utils.math.noise2d(v1.x, v1.y)
        const y = Utils.math.noise2d(v2.x, v2.y)
        const z = Utils.math.noise2d(v3.x, v3.y)
  ...
  geometry: {
    ...
    computeCurl: (x, y, z) => {
      ...
      let n1 = .noise2D(x, y + eps, z)
      let n2 = .noise2D(x, y - eps, z)
    ...
    randomPointInFace: (face, geometry) => {
      ...
      return GeometryrandomPointInTriangle(vA, vB, vC)
    },
    ...
    randomPointsInGeometry: (geometry, n) => {
      ...
      for (i = 0; i < n; i++) {
        ...
        result[i] = GeometryrandomPointInFace(faces[index], geometry)
    ...
    createCurves: () => {
      for (let i = 0; i < 10; i++) {
        const path = new THREE.CatmullRomCurve3(getCurve(new THREE.Vector3(i / 100, 0, 0)))
    ...
    randomPointsInBufferGeometry: (geometry, n) => {
      ...
      for (i = 0; i < n; i++) {
        ...
        //*  result[ i ] = GeometryrandomPointInFace( faces[ index ], geometry, true );
        vA.set(
          vertices[index * 9 + 0],
          vertices[index * 9 + 1],
          vertices[index * 9 + 2],
  ...
  tweakpane: {
    addScene: (scene, gui) => {
      ...
      folder.addInput(scene, 'fog', {
        label: 'Fog',
      })
      folder.addInput(scene.background, 'r', {
        label: 'Background color R',  
        min: 0,
        max: 1,
      })
      folder.addInput(scene.background, 'g', {
        label: 'Background color G',
        min: 0,
        max: 1,
      })
      folder.addInput(scene.background, 'b', {
        label: 'Background color B',
        min: 0,
        max: 1,
      })
      folder.addInput(scene.fog, 'near', {
        label: 'Fog near',
      })
      folder.addInput(scene.fog, 'far', {
        label: 'Fog far',
      })
      folder.addInput(scene.fog.color, 'r', {
        label: 'Fog color R',
        min: 0,
        max: 1,
      })
      folder.addInput(scene.fog.color, 'g', {
        label: 'Fog color G',
        min: 0,
        max: 1,
      })
      folder.addInput(scene.fog.color, 'b', {
        label: 'Fog color B',
        min: 0,
        max: 1,
      })
    },
    addCamera: (camera, gui) => {
      const folder = gui.addFolder({
        title: 'Camera',
      })

...
onMounted(() => {
  nextTick(() => {
    ...
    const geometr = new THREE.SphereGeometry(15, 100, 100)
    const aterial = new THREE.MeshBasicMaterial({ color: 0x8B0800 })
    planet = new THREE.Mesh(geometr, aterial)
    ...
    const smaterial = new THREE.ShaderMaterial({
      uniforms: {
        uTime: { value: 0 },
        uColor: { value: new THREE.Color(0x8B0800) },
      ...
      void main() {
        ...
        vec3 pos = position;
        pos += normal * sin(position.y + uTime) * 0.05;
      ...
      void main() {
        ...
        vec3 specular = pow(max(dot(reflectDir, viewDir), 0.0), 32.0) * specularStrength;
        vec3 diffuse = vec3(1.0);
        vec3 result = (specular + diffuse) * uColor;
    ...
    const material = new THREE.MeshBasicMaterial({ color: 0x8B0800 })
    ...
    scene.add(planet, mesh, ambientLight, camera, pointLight, spotLight, mousePointLight)

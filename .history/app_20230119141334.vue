...
onMounted(() => {
  nextTick(() => {
    ...
    const fillMeshWithSpheres = (mesh, count) => {
      const geometry = new THREE.SphereGeometry(0.1, 32, 32)
      const materialh = new THREE.MeshBasicMaterial({ color: 0x00FF00 })
      const group = new THREE.Group()
      for (let i = 0; i < count; i++) {
        const sphere = new THREE.Mesh(geometry, materialh)
        mesh.geometry.attribute.position[i].x = Math.random() * 100 - 50
        mesh.geometry.attribute.position[i].y = Math.random() * 100 - 50
        mesh.geometry.attribute.position[i].z = Math.random() * 100 - 50
        sphere.updateMatrix()
    ...
    const geometry = new THREE.TorusBufferGeometry(10, 3, 16, 18).toNonIndexed()
    // console.log(geometry);
    var material = new THREE.MeshBasicMaterial({ color: 0x404040, wireframe: true })
    const torus = new THREE.Mesh(geometry, material)
    scene.add(torus)

    const pointsGeom = fillMeshWithSpheres(torus, 10000)
    // console.log(pointsGeom);
    const pointsMat = new THREE.PointsMaterial({ color: 'aqua', size: 0.25 })
    ...
    const addNoiseToBufferGeometry = (geometry, noise) => {
    ...
    loader.load(logo, (gltf) => {
      ...
      model.children.map((letter) => {
        letter.geometry.position.map((point) => {
          addNoiseToBufferGeometry(letter.geometry, noise)
        })
      })

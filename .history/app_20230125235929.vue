...
import { useMagicKeyRegistration } from '~/composables/'
import EmberParticles from '~/particles/EmberParticles'

import { useMagicMouse } from '~/core/reti.js'
const { isPressed, isDown, isReleased } = useMagicKeys()
...
const tessellateGeometry = new THREE.BufferGeometry()
...
onMounted(() => {
  nextTick(() => {
    ...
    const debugEnv = new DebugEnvironment()

    scene.add(axesHelper, ...debugEnv.children)
    ...
    const u = y()
    //     debugger
    scene.add(emberParticles.points, ambientLight, camera, pointLight, spotLight, mousePointLight)

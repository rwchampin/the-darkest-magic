...
const b = () => {
  ...
  for (let i = 0; i < 2000; i++) {
    const vertex = new THREE.Vector3(radius, Math.random() * 2 - 1, Math.random() * 2 - 1)
  ...
  sphereGeometry.setDrawRange(0, vectors.length)
  ...
  function uo() {
    for (let i = 0, j = 0; i < vectors.length; i++, j += 3) {
      ...
      if (Date.now() > particle.delay) {
        particle.applyAxisAngle(particle.rotationAxis, 0.001 * i)
        sp[j] = particle.x
        sp[j + 1] = particle.y
        sp[j + 2] = particle.z
      }
...
const init = () => {
  ...
  *********************************/
  ...
  gsap.ticker.add((time, deltaTime, frame) => {
    ...
    // updateLightSphere()
    cube.position.x = radius * Math.random() * Math.sin(deltaTime * (Math.PI * 2))
    cube.position.y = radius * Math.random() * Math.cos(deltaTime * (Math.PI * 2))

const Vector = {
  ...
  getMass: (vector) => {
   ...
   getGravity: (vector1, vector2) => {
    /**
     * Returns the gravity vector between two vectors
     ...
     * const gravity = Vector.getGravity(vector1, vector2)
     * console.log(gravity.x, gravity.y)
     * */
     
    const force = this.subtract(vector2, vector1)
    const distance = this.distance(vector1, vector2)
    const strength = (G * this.getMass(vector2)) / (distance * distance)
    

  getAcceleration: (vector1, vector2, mass) => {
    /**
     * Returns the acceleration vector between two vectors
     * @param {Object} vector1 - {x, y}
     * @param {Object} vector2 - {x, y}
     * @param {Number} mass - Mass of the second vector
     * @return {Object} - {x, y}
     * @example
     * const vector1 = { x: 0, y: 0 }
     * const vector2 = { x: 100, y: 100 }
     * const mass = 100
     * const acceleration = Vector.getAcceleration(vector1, vector2, mass)

  ...
  attachHandlers() {
    window.addEventListener('mousemove', (event) => {
      ...
      for (let index = 0; index < this.intersects.length; index++) {
        const coords = this.intersects[index].point.copy()
        gsap.to(this.mouseLightPlane.position, { x: coords.x, y: coords.y, z: coords.z, duration: 0.5, ease: 'elastic.out(1, 0.3)' })
      }

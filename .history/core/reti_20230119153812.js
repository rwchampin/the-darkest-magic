mousePointLight = new THREE.PointLight(0xFF0000, 100, 100); (function (root, factory) {
  ...
  RETICOOL.prototype = {
    ...
    init() {
      ...
      if (this.config.cursor)
        document.documentElement.style.cursor = this.config.cursor
      ...
      if (this.config.useCSSVars && cssVarSupport)
        this.activateCSSVars()
    ...
    lock(x, y, w, h) {
      if (x !== undefined) {
        ...
        if (this.config.lockClass)
          this.$.classList.add(this.config.lockClass)
    ...
    unlock() {
      if (this.locked) {
        if (this.config.lockClass)
          this.$.classList.remove(this.config.lockClass)
        this.lx = this.x
        this.ly = this.y
        this.setColor(this.config.color, this.config.opacity)
      }

...
export default class MagicMouse {
  ...
  animate() {
    ...
    debugger
    if (!y || !that.temp || !that.eMouse || !that.elasticMouse || !that.elasticMouseVel) {
      throw createError({
    ...
    y.style.transform = `translate(${that.elasticMouse.x}px, ${that.elasticMouse.y}px)`
    ...
    that.mouseLight.position.copy(that.elasticMouse.position)

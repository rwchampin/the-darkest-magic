...
export default class MagicMouse {
  ...
  animate() {
    ...
    debugger
    if (!y || !self.temp || !self.eMouse || !self.elasticMouse || !self.elasticMouseVel) {
      throw createError({
    ...
    y.style.transform = `translate(${self.elasticMouse.x}px, ${self.elasticMouse.y}px)`

    self.temp.copy(self.eMouse).sub(self.elasticMouse).multiplyScalar(0.15)
    self.elasticMouseVel.add(self.temp)
    self.elasticMouseVel.multiplyScalar(0.85)
    self.elasticMouse.add(self.elasticMouseVel)

    self.mouseLight.position.copy(self.elasticMouse.position)

    self.material.uniforms.uLight.value = self.mouseLight.position
    self.material.uniforms.time.value = Date.now() * 0.0005

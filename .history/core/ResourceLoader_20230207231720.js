...
export default class ResourceLoader extends EventEmitter {
  ...
  constructor({nuxtApp, debugMode, appStore}) {
    ...
    this.lm = new THREE.LoadingManager((url, itemsLoaded, itemsTotal) => { 
      ...
      if (debugMode) {
        ...
        nuxtApp.$messgeApi.debug("RESOURCES LOADED")
          window.messageApi.success("RESOURCES LOADED")
      }
      window.$resources = this.items;
      nuxtApp.$resources = this.items;
      nuxtApp.provide("resources", this.items)
      ...
      this.trigger("assets:progress", progress)
    }, (url) => {
  ...
  setLoaders() {
    ...
    const cubeTextureLoader = new THREE.CubeTextureLoader(this.lm);
    this.loaders.push({
      extensions: ['png', 'jpg'],
      action: (_resource) => {

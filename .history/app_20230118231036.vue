...
import Stats from '~particles/core/vendor/stats.js'
...
let stats: { update: () => void; showPanel: (arg0: number) => void; dom: { style: { position: string; top: string; left: string; zIndex: string; width: number } } }, planet, stars, starlight, suniforms, scene: THREE.Scene, camera: THREE.PerspectiveCamera | THREE.Camera | THREE.Object3D<THREE.Event>, renderer: THREE.WebGLRenderer, ambientLight, pointLight, directionalLight, rectAreaLight, spotLight, hemisphereLight
...
onMounted(() => {
  nextTick(() => {
    stats = new Stats()
    planet = useBlackEnergy(nuxtApp).planet
    planet.material = new BlackGooBallMaterial()

    stars = useBlackEnergy(nuxtApp).stars

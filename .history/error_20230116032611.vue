...
onMounted(() => {
  ...
  const c1 = createCanvas({ width: w, height: size })
  const c2 = createCanvas({ width: w, height: size })
  const c3 = createCanvas({ width: w, height: size })
  ...
  function writeText(canvas, context, text) {
    ...
    for (let i = 0; i < lines.length; i++)
      context.fillText(lines[i], canvas.width / 2, canvas.height / 2 + lineheight * i - (lineheight * (lines.length - 1)))
  }
  ...
  function clear() {
    ...
    canvas.fillRect(100, 100, tela.width, tela.height)

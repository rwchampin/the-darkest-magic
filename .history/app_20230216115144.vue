...
const init = () => {
  ...
  ** FUCK START
  *********************************/
  const total = 100, radius = 3; let theta = 0, phi = 0
  for (let i = 0; i < total; i++) {
    const lng = mapRange(i, 0, total, -Math.PI, Math.PI);
    for (let j = 0; j < total; j++) {
      const lat = mapRange(i, 0, total, -Math.PI / 2, Math.PI / 2);

      const x = radius * Math.sin(lng) * Math.cos(lat)
      const y = radius * Math.sin(lng) * Math.cos(lat)
      const z = radius * Math.sin(lng) * Math.cos(lat)

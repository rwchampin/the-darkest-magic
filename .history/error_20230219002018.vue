...
onMounted(() => {
  ...
  const c1 = createCanvas({ width: window.innerWidth, height: 300 })
  const c2 = createCanvas({ width: window.innerWidth, height: 300 })
  const c3 = createCanvas({ width: window.innerWidth, height: 300 })
  ...
  function writeText(canvas, context, text) {
    const size = '50vh'
    context.font = `${size} Montserrat-Black`
  ...
  function maskCanvas() {
    c3.context.drawImage(c2.canvas, 0, 300, c2.canvas.width, 300)
    c3.context.globalCompositeOperation = 'source-atop'
    c3.context.drawImage(c1.canvas, 0, 0, c1.canvas.width, 300)

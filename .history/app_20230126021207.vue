...
const tessellateThatBitch = () => {
  ...
  const animateTessellate = () => {
    ...
    tessellateMaterial.uniforms.amplitude.value = Math.sin(time) * 0.5
  }
...
onMounted(() => {
  nextTick(() => {
    ...
    const { tessellateMesh, animateTessellate } = tessellateThatBitch()

    // tessellateMesh.scale.set(0.3, 0.3, 0.3)
    gsap.ticker.add(() => {
      // animateTessellate()
      animate()
    })
    ...
    tessellateMesh.position.set(0, 100, 50)
    gsap.to('.canvas-ui', {
      opacity: 1,
      duration: 1,
      ease: 'power2.out',

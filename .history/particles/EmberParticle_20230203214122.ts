...
const noise: Coords = Utils.noise.improvedNoise(Date.now)
...
const computeCurl = (x: number, y: number, z: number): {} => {
  ...
  const curl = {
    x: (n_y1 - n_y0 - n_z1 + n_z0) * (Math.random() * Math.sin()),
    y: (n_z1 - n_z0 - n_x1 + n_x0) * (Math.random() * Math.cos()),
    z: (n_x1 - n_x0 - n_y1 + n_y0) * (Math.random() * Math.sin()),
  }

  return curl
}
const positionPointsInCircle = () => {
...
export default class EmberParticle {
  ...
  init() {
    for (let i = 0; i < this.particleCount; i++) {
      ...
      const y = 0// this.height / 2
      const z = 0// this.depth / 2 - Math.random() * this.depth

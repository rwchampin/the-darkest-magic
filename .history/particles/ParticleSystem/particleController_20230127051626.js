...
export default function ParticleController() {
  ...
  let particleStyle = 'basic'
  let programmaticUpdate = false

  $(() => {
    const t = document.createElement('canvas')
    t.classList.add('canvas-ui test')
    t.style = { width: '100vw', height: '100vh', position: 'fixed', top: 0, left: 0, zIndex: 99999999999999 }
    display = new Display(t)
    display.init()
  ...
  function bindKeys() {
    $(window).keypress((evt) => {
      ...
      // eslint-disable-next-line no-unreachable
        case 32: displayPause(); return false; break
  ...
  function registerButtons() {
    ...
    $('#clickBehavior1').click(() => { particleSystem.mouseFieldStrength = -200 })
    $('#clickBehavior2').click(() => { particleSystem.mouseFieldStrength = 200 })
    $('#clickBehavior3').click(() => { particleSystem.mouseFieldStrength = 0 })
    ...
    // const shareUrl = 'http://www.facebook.com/share.php?u='
  ...
  function toggleInfo() { toggleDrawable(display, 'info') }
  function toggleObjects() { toggleDrawable(particleSystem, 'objects') }
  function toggleParticles() { toggleDrawable(particleSystem, 'particles') }
  function toggleVelocities() { toggleDrawable(particleSystem, 'velocities') }
  function toggleAccelerations() { toggleDrawable(particleSystem, 'accelerations') }
  ...
  function updateLabels() {
    let i = -1; const drawable = ['accelerations', 'objects', 'particles', 'velocities']
    while (drawable[++i])
  ...
  function objectClicked(evt) {
    ...
    if (object.constructor === ParticleEmitter) {
      ...
      $('.angleSlider').each((i, el) => {
        const angle = object.velocity.getAngleDegrees().toFixed(0) * Math.PI / 180
        //   console.log(angle)
        $(el).slider({
          ...
          max: 180,
          stop: updateHash,
          slide(evt, ui) { updateAngle(object, ui.value) },
        })
      })
      $('.velocitySlider').each((i, el) => {
        const magnitude = object.velocity.getMagnitude().toFixed(0)
        $(el).slider({
          value: magnitude,
          min: 0.1,
          max: 3,
          step: 0.2,
          stop: updateHash,
          slide(evt, ui) { updateVelocity(object, ui.value) },
        })
      })
      $('.spreadSlider').each((i, el) => {
        const angle = object.spread / Math.PI * 180
        $(el).slider({
          value: angle,
          min: 0,
          max: 180,
          stop: updateHash,
          slide(evt, ui) { updateSpread(object, ui.value) },
        })
      })
    }
    ...
    $('#deleteObject').unbind().click((/* evt */) => { deleteObject(object); hideFloatingControls() })
  }
  ...
  function loadCustomState(string) {
    ...
    display.draw.continuous = parts[0] === '1'

...
export default function ParticleSystem() {
  ...
  self.onAfterDraw = function (evt, display) {
    if (display.draw.info) {
      ...
      display.drawText(`Particles : ${self.getParticleCount()}`, new Vector(100, display.height - 10), 100)
    }
  }
  ...
  self.onMouseMove = function (evt, display) {
    self.mouseCoords = new Vector(evt.offsetX || (evt.layerX - display.canvas.offsetLeft), evt.offsetY || (evt.layerY - display.canvas.offsetTop))
    if (self.mouseField) {
  ...
  self.addNewParticles = function () {
    if (self.particles.length < self.maxParticles) {
      for (let i = 0, emitter; emitter = self.emitters[i]; i++) {
  ...
  self.drawParticles = function (display) {
    ...
    const { width, height } = display.width
    let particle; let i = -1
    while (particle = self.particles[++i])
      particle.draw(pixels, width, height)
  ...
  self.getObjectAtPoint = function (point) {
    for (let i = 0; i < self.emitters.length; i++) {

...
(function () {
  ...
  _.pluck = function (obj, key) {
    return _.map(obj, (value) => { return value[key] })
  }
  ...
  _.sortBy = function (obj, value, context) {
    const iterator = lookupIterator(value)
  ...
  const group = function (obj, value, context, behavior) {
    const result = {}
    const iterator = lookupIterator(value)
  ...
  _.uniq = _.unique = function (array, isSorted, iterator, context) {
    const initial = iterator ? _.map(array, iterator, context) : array
    const results = []
    const seen = []
  ...
  _.difference = function (array) {
    ...
    return _.filter(array, (value) => { return !_.contains(rest, value) })
  }
  ...
  _.zip = function () {
    ...
    const length = _.max(_.pluck(args, 'length'))
  ...
  _.lastIndexOf = function (array, item, from) {
    if (array == null)
      return -1
    const hasIndex = from != null
    if (nativeLastIndexOf && array.lastIndexOf === nativeLastIndexOf)
      return hasIndex ? array.lastIndexOf(item, from) : array.lastIndexOf(item)

    let i = (hasIndex ? from : array.length)
    while (i--) {
      if (array[i] === item)
        return i
    }
  ...
  _.bindAll = function (obj) {
    let funcs = slice.call(arguments, 1)
    if (funcs.length == 0)
  ...
  _.throttle = function (func, wait) {
    ...
    const whenDone = _.debounce(() => { more = throttling = false }, wait)
    return function () {
  ...
  _.times = function (n, iterator, context) {
    for (let i = 0; i < n; i++) iterator.call(context, i)
  }

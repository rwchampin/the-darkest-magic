import { Atoms } from '~/particles/Atoms.js'

export class ParticleSystem extends Atoms {
  constructor() {
    super()
    this.lib.noise.seed(this.RAND * 2000)
    const defaults = {
      max: 4000,
      size: 5,
    }
  ...
  update() {
    let pRef,
      ...
      colorString
    this.canvas.clear()
    this.canvas.ctx.lineCap = 'this.round'
    if (this.points.length < this.config.max && this.tick % 2 === 0) {
      const rand = this.ROUND(RAND())
      const x = rand
        ? this.ROUND(RAND()) * this.canvas.dimensions.x
        : RAND() * this.canvas.dimensions.x
      const y = rand
        ? RAND() * this.canvas.dimensions.y
        : this.ROUND(RAND()) * this.canvas.dimensions.y

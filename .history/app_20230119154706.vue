...
let sphere, mouse, raycaster, mousePointLight, scene, camera, renderer, ambientLight, pointLight, directionalLight, rectAreaLight, spotLight, hemisphereLight
...
onMounted(() => {
  nextTick(() => {
    ...
    function mouseMe() {
      ...
      const pplane = new THREE.Mesh(planeg, pmaterial)
      pplane.position.set(0, 0, 1)
      pplane.rotation.x = THREE.MathUtils.degToRad(0)
      scene.add(pplane)

      // add sphere
      const sgeometry = new THREE.SphereGeometry(1, 100, 100)
      const smaterial = new THREE.MeshBasicMaterial({ color: 0x000055 })
      const sphere = new THREE.Mesh(sgeometry, smaterial)
      sphere.position.set(0, 0, 0)
    ...
    scene.add(ambientLight, sphere, camera, pointLight, spotLight, mousePointLight)

...
onMounted(() => {
  ...
  const c1 = createCanvas({ width: '100%', height: 300 })
  const c2 = createCanvas({ width: '100%', height: 300 })
  const c3 = createCanvas({ width: '100%', height: 300 })
  ...
  function writeText(canvas, context, text) {
    ...
    for (let i = 0; i < lines.length; i++)
      context.fillText(lines[i], canvas.width * 0.5, canvas.height * 0.5 + lineheight * i - (lineheight * (lines.length - 1)))
  }

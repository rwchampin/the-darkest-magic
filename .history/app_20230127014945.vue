...
function createIcosahedron() {
  ...
  const icosGeometry = new THREE.IcosahedronGeometry(0.5, 20)
  ...
  const mesh = new THREE.Mesh(icosGeometry, icosMaterial)
  const animateIcosahedron = () => {
    mesh.rotation.x += 0.1
    mesh.rotation.y += 0.1
    mesh.material.uniforms.u_time.value += 0.4 * clock.getElapsedTime()
    mesh.material.uniforms.u_intensity.value = THREE.MathUtils.lerp(
      mesh.material.uniforms.u_intensity.value,
      hover ? 1 : 0.15,
      0.2,
    )
  }
  return { mesh as icosahedron, animateIcosahedron }
}
const animate = () => {
  animateIcosahedron()
...
onMounted(() => {
  nextTick(() => {
    ...
    ParticleController()
    const createIcosahedron = createIcosahedron()
    icosahedron = createIcosahedron.icosahedron
    animateIcosahedron = createIcosahedron.animateIcosahedron

...
const b = () => {
  ...
  const originalVertex = new THREE.Vector3(Math.PI * 2 * Math.random(), rand(), rand())
  const vectors = []
  const vertices = []
  const sizes = []
  const particlesDelay = 50
  for (let i = 0; i < 100; i++) {
    const vertex = new THREE.Vector3(originalVertex.randomDirection(), rand(), rand())
    vertex.rotationAxis = new THREE.Vector3(0, rand(), rand())

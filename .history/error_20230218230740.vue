...
onMounted(() => {
  const { scene, camera, renderer } = useCore()
  let particles = []
  const frequency = 20
  // Popolate particles
  setInterval(
  ...
  const c1 = Utils.createCanvas({ width: w, height: 300, id: 'error-canvas-1' })
  const c2 = Utils.createCanvas({ width: w, height: 300, id: 'error-canvas-2' })
  const c3 = Utils.createCanvas({ width: 500, height: 300, id: 'error-canvas-3' })
  ...
  function maskCanvas() {
    ...
    blur(c1.context, c1.canvas, Math.random() * 2)
  }

  ...
  const yo = () => {
    ...
    function draw() {
      ...
      for (let i = 0; i < maxParticles; i++) {
        ...
        const t = vertex.applyAxisAngle(new THREE.Vector3(0, Math.random() * 2 - 1, Math.random() * 2 - 1), Math.random() * Math.PI * 2);
        t.normalize();
        t.delay = Date.now() + (particlesDelay * i);
        positions.push(t);
      }
      sphereGeometry.setBufferAttribute('position', new THREE.BufferAttribute(new Float32Array(positions), 3));

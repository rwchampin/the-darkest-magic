...
let stats, scene, camera, renderer, updateParticles, updateLightSphere, sun, mercury
/*********************************
  * GLOBAL COMPONENTS
*********************************/
...
onMounted(() => {
  ...
  *********************************/
  ...
  scene = core.scene
  scene.fog = new THREE.Fog(0x999999, 1, 250)
  camera = core.camera
  ...
  const solar = usePoints({ scene, camera, renderer, nuxtApp })
  sun = solar.sun
  ...
  function animate() {
    ...
    //sphere position
    mercury.position.x = Math.sin(elapsed / .5) * 360;
    mercury.position.z = Math.cos(elapsed / .5) * 360;

...
void main(){
    ...
    noise=(1.*-waves)*turbulence(decay*abs(normal+time));
    qnoise=(2.*-eqcolor)*turbulence(decay*abs(normal+time));
    float b=pnoise(complex*(position)+vec3(1.*time),vec3(100.));
    
    if(fragment==true){
        displacement=-sin(noise)+normalize(b*.5);
    }else{
        displacement=-sin(noise)+cos(b*.5);
    }

...
(function (window, undefined) {
  ...
  const largeArraySize = 30

  /** Used to restore the original `_` reference in `noConflict` */
  const oldDash = window._

  /** Used to detect delimiter values that should be processed by `tokenizeEvaluate` */
  const reComplexDelimiter = /[-?+=!~*%&^<>|{(\/]|\[\D|\b(?:delete|in|instanceof|new|typeof|void)\b/
  ...
  const reInsertVariable = /(?:__e|__t = )\(\s*(?![\d\s"']|this\.)/g
  ...
  const noArgsClass = !isArguments(arguments)

  /** Detect if `Array#slice` cannot be used to convert strings to arrays (Opera < 10.52) */
  const noArraySliceOnStrings = slice.call('x')[0] != 'x'
   ...
   */
  const noCharByIndex = ('x'[0] + Object('x')[0]) != 'xx'
  ...
  const isBindFast = nativeBind && /\n|Opera/.test(nativeBind + toString.call(window.opera))

  /* Detect if `Object.keys` exists and is inferred to be fast (IE, Opera, V8) */
  const isKeysFast = nativeKeys && /^.+$|true/.test(nativeKeys + !!window.attachEvent)

  /* Detect if strict mode, "use strict", is inferred to be fast (V8) */
  const isStrictFast = !isBindFast
   ...
   */
  const iteratorTemplate = template(
  ...
  const filterIteratorOptions = {
    init: '[]',
    inLoop: 'callback(value, index, collection) && result.push(value)',
  }

  /** Reusable iterator options for `find`, `forEach`, `forIn`, and `forOwn` */
  const forEachIteratorOptions = {
    top: 'callback = createCallback(callback, thisArg)',
  }

  /** Reusable iterator options for `forIn` and `forOwn` */
  const forOwnIteratorOptions = {
    inLoop: {
  ...
  function cachedContains(array, fromIndex, largeSize) {
    ...
    const isLarge = (length - fromIndex) >= (largeSize || largeArraySize)
    const cache = isLarge ? {} : array
  ...
  function createBound(func, thisArg, partialArgs) {
    ...
    const isPartial = !partialArgs
    const methodName = func
    ...
    function bound() {
      ...
      let args = arguments
      let thisBinding = isPartial ? this : thisArg
      ...
      if (this instanceof bound) {
        ...
        const result = func.apply(thisBinding, args)
  ...
  function clone(value, deep, guard, stackA, stackB) {
    ...
    // inspect [[Class]]
    let isObj = objectTypes[typeof value]
    if (isObj) {
    ...
    // init cloned object
    const result = isArr ? ctor(value.length) : {}
    ...
    (isArr ? forEach : forOwn)(value, (objValue, key) => {
   ...
   */
  const defaults = createIterator(extendIteratorOptions, {
   ...
   */
  const functions = createIterator({
    useHas: false,
    args: 'object',
    init: '[]',
  ...
  function isEqual(a, b, stackA, stackB) {
    ...
    let isArr = arrayLikeClasses[className]
   ...
   */
  const keys = !nativeKeys ? shimKeys : function (object) {
   ...
   */
  const merge = createIterator(extendIteratorOptions, {
   ...
   * // => { 'name': 'moe' }
   */
  const omit = createIterator(omitIteratorOptions)
   ...
   * // => { 'name': 'moe' }
   */
  const pick = createIterator(omitIteratorOptions, {
    top:
   ...
   * // => { '3': 2, '5': 1 }
   */
  const countBy = createIterator(baseIteratorOptions, countByIteratorOptions)
   ...
   * // => false
   */
  const every = createIterator(baseIteratorOptions, everyIteratorOptions)
   ...
   * // => [2, 4, 6]
   */
  const filter = createIterator(baseIteratorOptions, filterIteratorOptions)
   ...
   * // => 2
   */
  const find = createIterator(baseIteratorOptions, forEachIteratorOptions, {
    init: 'undefined',
   ...
   * // => { '3': ['one', 'two'], '5': ['three'] }
   */
  const groupBy = createIterator(baseIteratorOptions, countByIteratorOptions, {
    inLoop:
      'var prop = callback(value, index, collection);\n'
   ...
   * // => [['1', '2', '3'], ['4', '5', '6']]
   */
  const invoke = createIterator(mapIteratorOptions, {
   ...
   */
  const map = createIterator(baseIteratorOptions, mapIteratorOptions)
  ...
  function max(collection, callback, thisArg) {
    ...
    if (callback || length !== +length) {
      ...
      forEach(collection, (value, index, collection) => {
        const current = callback(value, index, collection)
        if (current > computed) {
          computed = current
          result = value
        }
   ...
   * // => ['moe', 'larry', 'curly']
   */
  const pluck = createIterator(mapIteratorOptions, {
  ...
  function reduceRight(collection, callback, accumulator, thisArg) {
    let iteratee = collection
    let length = collection ? collection.length : 0
    let noaccum = arguments.length < 3
    ...
    forEach(collection, (value, index, object) => {
   ...
   * // => [1, 3, 5]
   */
  const reject = createIterator(baseIteratorOptions, filterIteratorOptions, {
  ...
  function shuffle(collection) {
    let index = -1
    const result = Array(collection ? collection.length : 0)

    forEach(collection, (value) => {
      const rand = floor(nativeRandom() * (++index + 1))
   ...
   * // => true
   */
  const some = createIterator(baseIteratorOptions, everyIteratorOptions, {
    init: 'false',
   ...
   * // => ['moe', 'larry', 'brendan']
   */
  const sortBy = createIterator(baseIteratorOptions, countByIteratorOptions, mapIteratorOptions, {
    inLoop: {
      array:
        'result[index] = {\n'
   ...
   * // => [{ 'name': 'moe', 'age': 40 }]
   */
  const where = createIterator(filterIteratorOptions, {
  ...
  function difference(array) {
    ...
    let index = -1
    const length = array.length
    const flattened = concat.apply(ArrayProto, arguments)
    const contains = cachedContains(flattened, length)
  ...
  function intersection(array) {
    ...
    array: while (++index < length) {
      ...
      if (indexOf(result, value) < 0) {
        for (let argsIndex = 1; argsIndex < argsLength; argsIndex++) {
  ...
  function range(start, end, step) {
    ...
    let index = -1
    const length = nativeMax(0, ceil((end - start) / step))
  ...
  function uniq(array, isSorted, callback, thisArg) {
    ...
    callback = createCallback(callback, thisArg)
    while (++index < length) {
      const computed = callback(array[index], index, array)
      if (isSorted
  ...
  function without(array) {
    let index = -1
    const length = array ? array.length : 0
    const contains = cachedContains(arguments, 1, 20)
    const result = []
  ...
  function zip(array) {
    let index = -1
    const length = array ? max(pluck(arguments, 'length')) : 0
    const result = Array(length)
  ...
  function delay(func, wait) {
    ...
    return setTimeout(() => { return func.apply(undefined, args) }, wait)
  }
  ...
  function defer(func) {
    ...
    return setTimeout(() => { return func.apply(undefined, args) }, 1)
  }
  ...
  function memoize(func, resolver) {
    const cache = {}
    return function () {
      const prop = resolver ? resolver.apply(this, arguments) : arguments[0]
  ...
  function mixin(object) {
    forEach(functions(object), (methodName) => {
      const func = lodash[methodName] = object[methodName]

      lodash.prototype[methodName] = function () {
        const args = [this.__wrapped__]
        if (arguments.length)
          push.apply(args, arguments)

        let result = func.apply(lodash, args)
        if (this.__chain__) {
  ...
  function result(object, property) {
    ...
    const value = object ? object[property] : null
  ...
  function template(text, data, options) {
    ...
    let source = '__p += \''
    let variable = options.variable || settings.variable
    ...
    text.replace(reDelimiters, (match, escapeValue, interpolateValue, evaluateValue, offset) => {
    ...
    if (!hasVariable) {
      ...
      if (isEvaluating) {
        ...
        const reDoubleVariable = RegExp(`(\\(\\s*)${variable}\\.${variable}\\b`, 'g')
        source = source
  ...
  forEach(['pop', 'push', 'reverse', 'shift', 'sort', 'splice', 'unshift'], (methodName) => {
  ...
  forEach(['concat', 'join', 'slice'], (methodName) => {
    const func = ArrayProto[methodName]

    lodash.prototype[methodName] = function () {
      const value = this.__wrapped__
      let result = func.apply(value, arguments)
